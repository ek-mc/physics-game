...
function genKinematicsExamStyleSet(){
 ...
 for (const a of [1,2,3,4]){
  for (const t of [10,12,15,18]){
   ...
   out.push(makeQuestion('kinematics','hard',
    `Στην ίδια κίνηση, ποια απόσταση διανύεται σε t=${t} s;`,
 ...
 for (const vmax of [12,16,20,24]){
  for (const tmax of [6,8,10,12]){
   ...
   out.push(makeQuestion('kinematics','hard',
    `Στο ίδιο γράφημα, ποια είναι η επιτάχυνση;`,
 ...
 for (const rpm of [600,720,780,900]){
  for (const diam of [0.5,0.66,0.8]){
   ...
   out.push(makeQuestion('kinematics','hard',
    `Στην ίδια κίνηση, ποια είναι η κεντρομόλος επιτάχυνση;`,
 ...
 for (const v0 of [10,12,14,16]){
  ...
  for (const ang of [30,37,45,53,60]) {
   ...
   out.push(makeQuestion('kinematics','hard',
    `Στην ίδια βολή (φ=${ang}°), ποιο είναι το μέγιστο ύψος;`,
...
function genRotationWorksheetStyle(){
 ...
 out.push(makeQuestion('rotation','veryhard',
  'Με την παραπάνω α(t), ποια μορφή έχει η θ(t) αν θ(0)=θ₀;',
 ...
 out.push(makeQuestion('rotation','hard',
  'Για το ίδιο σύστημα μαζών, ποια ροπή αδράνειας είναι συνήθως μεγαλύτερη: γύρω από άξονα z (κάθετο στο επίπεδο) ή γύρω από y (εντός επιπέδου);',
  ...
  for (const a1 of [2,3,4]){
   for (const t1 of [3,4,5]){
    ...
    for (const a2 of [-2,-3]){
     ...
     out.push(makeQuestion('kinematics','veryhard',
      `Σώμα επιταχύνεται για t₁=${t1}s με a₁=${a1} και μετά επιβραδύνεται για t₂=${t2}s με a₂=${a2}. Ποια είναι η τελική ταχύτητα;`,
      [v2, v1, v0, v1+Math.abs(a2)*t2].map(z=>`${fmt(z)} m/s`),
      0,
      'Δύο στάδια: πρώτα v₁=v₀+a₁t₁, μετά v₂=v₁+a₂t₂.'
     ));
     out.push(makeQuestion('kinematics','veryhard',
      `Στο ίδιο δι-στάδιο σενάριο, ποια είναι η συνολική απόσταση;`,
