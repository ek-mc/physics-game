...
function genWaves(){
 ...
 // Hard numeric/logic set for better variety
 for (const v of [120, 180, 240, 300]) {
  for (const l of [0.4, 0.6, 0.8, 1.2]) {
   const f = v / l;
   out.push(makeQuestion('waves','hard',
    `Κύμα με ταχύτητα v=${fmt(v)} m/s και μήκος κύματος λ=${fmt(l)} m. Ποια είναι η συχνότητα;`,
 ...
 for (const f of [5, 10, 20, 25]) {
  for (const T of [0.05, 0.1, 0.2]) {
   const ok = Math.abs(f*T - 1) < 1e-9;
   out.push(makeQuestion('waves','hard',
    `Για κύμα με f=${fmt(f)} Hz και T=${fmt(T)} s, ποιος έλεγχος είναι σωστός;`,
    [ok ? 'Ισχύει f·T=1' : 'ΔΕΝ ισχύει f·T=1', 'Ισχύει f=T', 'Ισχύει f+T=1', 'Ισχύει f/T=1'],
    0,
    'Πάντα ισχύει f=1/T, άρα f·T=1.'
   ));
  }
 }

 out.push(makeQuestion('waves','hard',
  'Αν σε ίδιο μέσο η συχνότητα γίνει τριπλάσια, τότε το μήκος κύματος γίνεται:',
  ['στο 1/3', 'τριπλάσιο', 'ίδιο', 'εξαρτάται από το πλάτος'],
  0,
  'Σε ίδιο μέσο v σταθερή, άρα από v=fλ προκύπτει λ∝1/f.'
 ));

 out.push(makeQuestion('waves','hard',
  'Ποιο μέγεθος καθορίζεται πρωτίστως από την πηγή και όχι από το μέσο;',
  ['Η συχνότητα f', 'Η ταχύτητα διάδοσης v', 'Η πυκνότητα του μέσου', 'Η ελαστική σταθερά του μέσου'],
  0,
  'Η συχνότητα ορίζεται από την πηγή. Η ταχύτητα διάδοσης από το μέσο.'
 ));

 out.push(makeQuestion('waves','hard',
  'Σε χορδή δημιουργείται εγκάρσιο κύμα. Αν αυξηθεί η τάση της χορδής, τότε γενικά η ταχύτητα διάδοσης:',
  ['Αυξάνεται', 'Μειώνεται', 'Μένει ίδια', 'Μηδενίζεται'],
  0,
  'Σε χορδή, μεγαλύτερη τάση οδηγεί σε μεγαλύτερη ταχύτητα διάδοσης.'
 ));

 out.push(makeQuestion('waves','hard',
  'Ποια δήλωση είναι σωστή για την ενέργεια κύματος;',
  ['Μεταφέρεται μέσω της διαταραχής χωρίς μεταφορά ύλης', 'Μεταφέρεται μόνο αν μεταφέρεται και ύλη', 'Δεν μεταφέρεται ποτέ', 'Εξαρτάται μόνο από το μήκος κύματος'],
  0,
  'Βασική ιδιότητα κυμάτων: μεταφορά ενέργειας/ορμής, όχι ύλης.'
 ));

 out.push(makeQuestion('waves','hard',
  'Σε διαμήκες κύμα, τα πυκνώματα και αραιώματα είναι:',
  ['Περιοχές αυξημένης/μειωμένης πυκνότητας κατά τη διάδοση', 'Κορυφές και κοιλίες εγκάρσιου κύματος', 'Σημεία μηδενικής ταχύτητας διάδοσης', 'Πάντα ακίνητα σημεία'],
  0,
  'Αυτός είναι ο φυσικός ορισμός των πυκνωμάτων/αραιωμάτων.'
 ));
